...
export default async function AnalyticsDataPage() {
  ...
  // Fetch properties on the server and pass to client dashboard
  ...
  try {
    ...
    const res = await fetch(`${baseUrl}/api/analytics/properties?limit=100`, {
      cache: "no-store",
      headers: {
        // Forward cookies so the API route can read the session
  ...
  return <PropertyAnalyticsDashboard properties={properties} />;
}
