...
function PropertyRow({ property, startDate, endDate, refreshKey, isSelected, onClick, isFavorite, onToggleFavorite, organicOnly }: {
  ...
  if (isLoading) return (
    <tr className="animate-pulse">
      ...
      {defaultMetrics.map(m => <td key={m} className="px-6 py-4 whitespace-nowrap"><div className="h-4 bg-muted rounded w-1/2"></div></td>)}
    </tr>
  );

  if (error) return (
    <tr>
      <td className="px-6 py-4 whitespace-nowrap text-sm text-destructive">{property.displayName} - Error loading</td>
      {defaultMetrics.map(metric => <td key={metric} className="px-6 py-4 whitespace-nowrap text-sm text-destructive/70">--</td>)}
    </tr>
  );

  return (
    <tr className={`cursor-pointer hover:bg-accent transition-colors ${isSelected ? 'bg-primary/10 border-l-4 border-primary' : ''}`} onClick={onClick}>
      <td className="px-6 py-4 whitespace-nowrap">
        <div className="text-sm font-medium text-foreground truncate max-w-xs" title={property.displayName}>{property.displayName}</div>
        <div className="text-xs text-muted-foreground truncate max-w-xs">{property.propertyId}</div>
        <button type="button" onClick={(e) => { e.stopPropagation(); onToggleFavorite(); }} aria-label={isFavorite ? "Unpin favorite" : "Pin as favorite"} className={`mt-2 inline-flex items-center text-sm ${isFavorite ? "text-yellow-500 dark:text-yellow-400" : "text-muted-foreground hover:text-foreground"}`} title={isFavorite ? "Unpin favorite" : "Pin as favorite"}>
...
export function ExcelTable({ properties, startDate, endDate, refreshKey, selectedPropertyIndex, onPropertySelect, favorites, onToggleFavorite, organicOnly }: {
  ...
  return (
    <div className="bg-card border border-border rounded-lg shadow-lg overflow-hidden">
      <div className="overflow-auto max-h-[70vh]">
        <table className="min-w-full divide-y divide-border">
          <thead className="bg-muted/50 sticky top-0 z-20">
            <tr>
              <th className="px-6 py-3 text-left text-xs font-medium text-muted-foreground uppercase tracking-wider sticky left-0 bg-muted/50 z-30 min-w-[200px]">Brand / Property</th>
              {defaultMetrics.map(metric => {
                const colors = getMetricColors(metric);
                return (<th key={metric} className={`px-6 py-3 text-left text-xs font-medium uppercase tracking-wider sticky top-0 bg-muted/50 z-20 ${colors.label} ${colors.border} border-l min-w-[120px]`}>{getMetricLabel(metric)}</th>);
              })}
            </tr>
          </thead>
      ...
      {properties.length === 0 && (<div className="text-center py-12"><p className="text-muted-foreground">No properties found</p></div>)}
    </div>
  );
}
