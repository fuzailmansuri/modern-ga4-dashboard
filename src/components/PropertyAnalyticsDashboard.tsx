...
// Fixed metrics (must be valid GA4 metric names)
// Align with backend defaults to avoid invalid metric errors
const defaultMetrics = ['activeUsers', 'newUsers', 'sessions', 'screenPageViews', 'bounceRate', 'averageSessionDuration'];
...
// Single property data hook with 10-minute cache
function usePropertyData(
  ...
  const { data, error, isLoading } = useSWR<PropertyDataResponse>(
    ...
    () => fetcher<PropertyDataResponse>(`/api/analytics/properties/${property.propertyId}/data?startDate=${startDate}&endDate=${endDate}&metrics=${metricsQuery}${organicOnly ? '&organicOnly=1' : ''}`),
    {
...
// Extract and format metrics from API response
function extractMetrics(resp?: PropertyDataResponse): Record<string, number> {
    ...
    defaultMetrics.forEach((metric, idx) => {
      if (values[idx]?.value !== undefined) {
        ...
        if (metric === 'bounceRate') {
          metrics[metric] = Math.max(metrics[metric] ?? 0, val);
        } else {
          metrics[metric] = (metrics[metric] ?? 0) + val;
        }
      }
    });
  }
...
// Individual Property Row Component for Excel View
function PropertyRow({ 
    ...
    return (
      <tr>
        <td className="px-6 py-4 whitespace-nowrap text-sm text-red-600">
          {property.displayName} - Error loading
        </td>
        {defaultMetrics.map(metric => (
          <td key={metric} className="px-6 py-4 whitespace-nowrap text-sm text-red-400">
            --
          </td>
        ))}
      </tr>
  ...
  return (
    <tr 
      className={`cursor-pointer hover:bg-gray-50 transition-colors ${isSelected ? 'bg-blue-50 border-l-4 border-blue-400' : ''}`}
      onClick={onClick}
    >
      <td className="px-6 py-4 whitespace-nowrap">
        <div className="text-sm font-medium text-gray-900 truncate max-w-xs" title={property.displayName}>
        ...
        <button
          ...
          className={`mt-2 inline-flex items-center text-sm ${isFavorite ? "text-yellow-500" : "text-gray-400 hover:text-gray-600"}`}
      ...
      {defaultMetrics.map(metric => {
        ...
        return (
          <td key={metric} className={`px-6 py-4 whitespace-nowrap text-sm font-semibold ${colors.number}`}>
            {formatValue(metric, metrics[metric] ?? 0)}
          </td>
        );
      })}
    </tr>
...
// Excel-like Table Component with Fixed Header
function ExcelTable({ 
  properties, 
  startDate, 
  endDate, 
      ...
      <div className="overflow-auto max-h-[70vh]">
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50 sticky top-0 z-20">
            <tr>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider sticky left-0 bg-gray-50 z-30 min-w-[200px]">
                Brand / Property
              </th>
              {defaultMetrics.map(metric => {
                const colors = getMetricColors(metric);
                return (
                  <th 
                    key={metric} 
                    className={`px-6 py-3 text-left text-xs font-medium uppercase tracking-wider sticky top-0 bg-gray-50 z-20 ${colors.label} ${colors.border} border-l min-w-[120px]`}
                  >
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {properties.map((property, index) => (
              <PropertyRow
                ...
                onToggleFavorite={() => onToggleFavorite(property.propertyId)}
      ...
      <div className="bg-white rounded-lg shadow p-6">
        <div className="flex items-center justify-between mb-2">
          ...
          <button
            ...
            className={`inline-flex items-center text-sm ${isFavorite ? "text-yellow-500" : "text-gray-400 hover:text-gray-600"}`}
            aria-label={isFavorite ? "Unpin favorite" : "Pin as favorite"}
            title={isFavorite ? "Unpin favorite" : "Pin as favorite"}
          >
  ...
  return (
    <div className="bg-white rounded-lg shadow">
      <div className="flex items-center justify-between p-4">
        <h3 className="text-lg font-medium text-gray-900">
          {property.displayName} <span className="text-xs text-gray-500">({property.propertyId})</span>
        </h3>
        <button
          type="button"
          onClick={() => onToggleFavorite()}
...
export function PropertyAnalyticsDashboard({
  ...
  if (!properties?.length) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-gray-50">
        <div className="text-center">
          <p className="text-gray-600">No properties available to display analytics data.</p>
        </div>
      </div>
    );
  }
          ...
          <div className="rounded-lg bg-white p-4 shadow">
            <div className="flex flex-col gap-4">
              <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  <h3 className="mb-4 text-lg font-medium text-gray-900">Date Range & Controls</h3>
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="inline-flex items-center rounded-md border border-transparent bg-green-600 px-4 py-2 text-sm font-medium text-white hover:bg-green-700 focus:ring-2 focus:ring-green-500 focus:ring-offset-2 focus:outline-none transition-colors"
                  >
                  ...
                  <div className="flex items-center gap-2">
                    <label htmlFor="property-limit" className="text-sm text-gray-700">Show</label>
                    <select
                      id="property-limit"
                      value={limit}
              ...
              <div className="flex items-center justify-between border-t border-gray-200 pt-4">
                <div className="flex items-center gap-6 flex-wrap">
                  ...
                  />
                  <label htmlFor="hide-failed-dashboards" className="ml-2 text-sm text-gray-700">
                  ...
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      className="h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                    />
                    <label htmlFor="organic-only" className="text-sm text-gray-700">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    {favorites.size > 0 && (
                      <div className="flex items-center gap-2">
                        ...
                        />
                        <label htmlFor="favorites-only" className="text-gray-600">Show favorites only</label>
                      </div>
                    )}
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        className="inline-flex items-center gap-1.5 px-2.5 py-1.5 text-xs font-medium text-gray-600 bg-white border border-gray-300 rounded-md hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"
                      ...
                      <button
                        ...
                        className="inline-flex items-center gap-2 px-3 py-1.5 text-xs font-medium text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed"
